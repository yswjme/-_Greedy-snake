...
void circle_full_color(uint16_t x0,uint16_t y0,uint8_t r,uint16_t color);							//填充圆形色块
void LCD_Draw_Picture(uint16_t x,uint16_t y,uint16_t col,uint16_t row,unsigned short *pic);		//显示图片
void Chinese_Show_one(uint8_t x, uint16_t y, uint8_t num, uint8_t size, uint8_t mode);			//汉字
